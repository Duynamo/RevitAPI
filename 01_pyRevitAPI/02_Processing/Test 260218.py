...
# =====================================================================
# FIX 1: Khởi tạo excelFile = None ở module level để tránh NameError
# =====================================================================
excelFile = None
...
# =====================================================================
# FIX 2: Thêm setter cho tất cả các property của viewModel
...
class MyWindow(Window):
    ...
    def _select_row_from_checkbox(self, checkbox):
        try:
            ...
            print("Error _select_row_from_checkbox: {}".format(str(ex)))

    def update_listview(self):
        if not self.lvElements:
            return
        
        # Cập nhật số thứ tự No — hoạt động được nhờ FIX 2 (setter)
    ...
    def lvElements_PreviewMouseLeftButtonDown(self, sender, e):
        """Click vào bất kỳ vị trí nào trong hàng → toggle checkbox"""
        try:
            source = e.OriginalSource

            # Nếu click trực tiếp vào CheckBox → bỏ qua, để checkbox tự xử lý
            parent_check = source
            ...
            # Lấy viewModel từ DataContext của row
            vm = item.DataContext
            if vm is not None and hasattr(vm, 'gvcSelectID'):
                vm.gvcSelectID = not vm.gvcSelectID  # toggle True/False
                self.lvElements.Items.Refresh()       # refresh để UI cập nhật checkbox

        except Exception as ex:
            # print("Toggle checkbox error: {}".format(str(ex)))
            pass
