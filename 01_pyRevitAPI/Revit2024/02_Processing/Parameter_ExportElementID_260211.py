...
def pick_elements_in_order():
    # filter = CategoryFilter(CATEGORIES)
    selected_elements = []
    instruction = "Chọn lần lượt các đối tượng (Pipes, Fittings, Accessories, Generic Models). Nhấn Esc để kết thúc."

    while True:
        try:
            from Autodesk.Revit.UI.Selection import ObjectType
            ref = uidoc.Selection.PickObject(ObjectType.Element, instruction)
            if ref:
                elem = doc.GetElement(ref)
                if elem:
                    selected_elements.append(elem)
                    # Optional: hiển thị thông báo nhỏ mỗi lần pick (có thể bỏ)
                    # TaskDialog.Show("Đã pick", f"Element ID: {elem.Id.IntegerValue}")
        except Exception as ex:
            # OperationCanceledException khi nhấn Esc hoặc Cancel
            if "OperationCanceledException" in str(ex):
                break
            else:
...
if excel_path:
    elements = pick_elements_in_order()
    if elements:
