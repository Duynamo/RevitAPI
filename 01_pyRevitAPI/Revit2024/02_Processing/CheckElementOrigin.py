...
"""_______________________________________________________________"""
def calculateRotationAngle(R):
	trace = sum(R[i][i] for i in range(3))
	traceMinus1 = min(max(trace -1,-1), 1)
	theta = math.acos(traceMinus1/2)
	return math.degrees(theta)
"""_______________________________________________________________"""
def calculateRotationAxis(R):
	theta = calculateRotationAngle(R)
	sin_theta_rad = math.sin(theta)
	u = [-1/(2*sin_theta_rad)*(R[2][1] - R[1][2]),
	     -1/(2*sin_theta_rad)*(R[0][2] - R[2][0]), 
	     -1/(2*sin_theta_rad)*(R[1][0] - R[0][1])] 
	return u, sin_theta_rad
"""_______________________________________________________________"""
class MainForm(Form):
	def __init__(self):
	...
	def Btt_pickElementClick(self, sender, e):
		...
		for ele in desElements:
			...
			transOriginX = transform.Origin.X
			transOriginY = transform.Origin.Y
			transOriginZ = transform.Origin.Z
			...
			rotation_Axis  = calculateRotationAxis([list(basicX_scaled), list(basicY_scaled), list(basicZ_scaled)])
			rotation_angle = calculateRotationAngle([list(basicX_scaled), list(basicY_scaled), list(basicZ_scaled)])

			self._txb_originX = str(transform.Origin)
			self._txb_originY = str(transOriginY)
			self._txb_originZ = str(transOriginZ)

			self._txb_theta.Text = str(rotation_angle)
			self._txb_Ux.Text    = str(rotation_Axis[0][0])
			self._txb_Uy.Text    = str(rotation_Axis[0][1])
			self._txb_Uz.Text    = str(rotation_Axis[0][2])
